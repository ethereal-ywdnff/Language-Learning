{- trees.hs - functions to work with binary trees, as described in lecture slides
 - Written by Emma Norling, September 2019
 -}
data Tree a = Empty | Node (Tree a) a (Tree a) -- deriving Show -- Instead of deriving Show, use instance of Show as below
...
successor val (Node left root right)
    ...
    | val < root  = if isEmpty left || val >= (maximum' left)
                        then root else successor val left
    | otherwise = successor val right

predecessor :: Ord a => a -> Tree a -> a
predecessor val (Node left root right)
    | val == root = maximum' left
    | val > root  = if isEmpty right || val <= (minimum' right)
...
[ name | (name, n) <- sortBy cmp (zip names (randoms (mkStdGen 1234) :: [Int])) ]
